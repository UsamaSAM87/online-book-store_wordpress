...
class WC_Advanced_Shipment_Tracking_Admin {
	...
	*/
	function get_ast_product_license_data(){
		$form_data = array(			
			...
			'ast_product_license_email' => array(
				'type'		=> 'text',
				'title'		=> __( 'Email', 'woo-advanced-shipment-tracking' ),						
				'show' => true,
				'tooltip' => __( 'Please provide your registered email with us', 'woo-advanced-shipment-tracking' ),
				'class'     => '',
			),			
		);
		return $form_data;
	...
	*/
	function ast_product_license_activate_callback(){
		...
		if ( ! empty( $_POST ) && check_admin_referer( 'wc_ast_addons_form', 'wc_ast_addons_form' ) ) {
			...
			$return = $this->license_authorize_action( sanitize_text_field($_POST['ast_product_license_key']), sanitize_email($_POST['ast_product_license_email']), 'activate' );
						
			if($return->success == 'true'){
				update_option( $this->license_key, sanitize_text_field($_POST['ast_product_license_key']));
				update_option( $this->license_email, sanitize_email($_POST['ast_product_license_email']));
			}
	...
	*/
	function ast_product_license_deactivate_callback(){
		...
		if ( ! empty( $_POST ) && check_admin_referer( 'wc_ast_addons_form', 'wc_ast_addons_form' ) ) {
			...
			if(isset($_POST['ast_product_license_email']) && isset($_POST['ast_product_license_key'])){
				$return = $this->license_authorize_action( sanitize_text_field($_POST['ast_product_license_key']), sanitize_email($_POST['ast_product_license_email']), 'deactivate' );
				if($return->success == 'true'){
					update_option( $this->license_key, '');
					update_option( $this->license_email, '');
	...
	*/
	public function license_authorize_action( $purchase_code = '', $email = '', $action = 'validate' ) {
		global $wp_version;
		...
		//echo '<pre>';print_r($authorize_data);echo '</pre>';exit;
	...
	*/
	public function check_license_valid($action = 'validate') {
		...
		$authorize = $this->license_authorize_action($serial, $email, $action);
	...
	*/
	function ast_product_licence_notice() {
		...
		$message = __( 'Opps! your <strong>Tracking Per Item Add-on for AST</strong> licence is not activated. To buy license <a href="https://www.zorem.com/shop/tracking-per-item-ast-add-on/" target="_blank">click here.</a>', 'woo-advanced-shipment-tracking' );
