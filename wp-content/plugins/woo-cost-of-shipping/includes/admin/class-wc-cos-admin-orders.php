...
class WC_COS_Admin_Orders {
	 ...
	 * This function will return success on successful update_post_meta for the new shipping cost and method.
	 * 
	 * @since 1.0.0
	 * 
	...
	function set_shipping_cost_callback() {
		...
		do_action( 'wc_cos_before_ajax_set_shipping_cost', $_POST['post_id'], $_POST['shipping_cost'], 'manual' );
		
		// Returns true on success, false on anything else
		if( $this->store_cost_of_shipping( $_POST['post_id'], $_POST['shipping_cost'], 'manual' ) ){
			
			do_action( 'wc_cos_after_ajax_set_shipping_cost', $_POST['post_id'], $_POST['shipping_cost'], 'manual' );
	...
	public function store_cost_of_shipping( $order_id, $cos, $method ) {
		...
		if ( true === $this->plugin->__get( 'debug' ) ) {
			$this->log->add( 'wc-cost-of-shipping', __( 'Function: store_cost_of_shipping', 'woocommerce-cost-of-shipping' ) );
			$this->log->add( 'wc-cost-of-shipping', sprintf( __( '   Order cost of shipping being stored to order id %d', 'woocommerce-cost-of-shipping' ), $order_id ) );
			$this->log->add( 'wc-cost-of-shipping', sprintf( __( '       Cost of shipping: %.2f via method: %s', 'woocommerce-cost-of-shipping' ), $cos, $method ) );
		}
		$cos = apply_filters( 'wc_cos_store_cost_of_shipping', $cos );
		$method = apply_filters( 'wc_cos_store_cost_of_shipping_method', $method );
		 ...
		 */
		$flag = update_post_meta( $order_id, '_wc_cost_of_shipping', (float) $cos );
		if( !$flag ) {
			if ( true === $this->plugin->__get( 'debug' ) ) {
				$this->log->add( 'wc-cost-of-shipping', __( '       No need to update Cost of Shipping, values are equivalent.', 'woocommerce-cost-of-shipping' ) );
			}
			return false;
		}

		update_post_meta( $order_id, '_wc_cos_method', $method );

		if ( true === $this->plugin->__get( 'debug' ) ) {
			$this->log->add( 'wc-cost-of-shipping', __( '     Cost of shipping value and method updated.', 'woocommerce-cost-of-shipping' ) );
		}
	...
	public function maybe_save_order_shipping_cost_shipstation( $order, $args ) {
		...
		if( !isset($xml) ) {
			...
			$this->log->add( 'wc-cost-of-shipping', __( 'shipstation integration xml not set', 'woocommerce-cost-of-shipping' ) );
		}
		elseif( empty($xml) ) {
			$this->log->add( 'shipstation', __( 'shipstation integration xml is empty', 'woocommerce-cost-of-shipping' ) );
			$this->log->add( 'wc-cost-of-shipping', __( 'shipstation integration xml is empty', 'woocommerce-cost-of-shipping' ) );
		}
		elseif( !isset($xml->ShippingCost) ) {
			...
			$this->log->add( 'wc-cost-of-shipping', __( 'shipstation integration xml does not have shipping amount set', 'woocommerce-cost-of-shipping' ) );
		}
		else {
			$shipping_cost = wc_clean( $xml->ShippingCost );
			$this->store_cost_of_shipping($order->get_id(), $shipping_cost, 'shipstation' );
			// update_post_meta( $order->get_id(), '_wc_cost_of_shipping', (float) $shipping_cost );
			// update_post_meta( $order->get_id(), '_wc_cos_method', 'shipstation' );
		}
	...
	public function maybe_save_order_shipping_cost_woocommerce_services( $order_id = 0 ) {
		...
		if( true === is_woocommerce_services_active() ) {
			if ( true === $this->plugin->__get( 'debug' ) )
				$this->log->add( 'wc-cost-of-shipping', __( 'WooCommerce services is active, retrieving labels.', 'woocommerce-cost-of-shipping' ) );
			$labels = $this->get_wcs_label_order_meta_data( $order_id );
			$cost_of_shipping = 0.00;
			foreach ( $labels as $label ) {
				$cost_of_shipping += (float)$label['rate'];
			}
			...
			if ( true === $this->plugin->__get( 'debug' ) )
				$this->log->add( 'wc-cost-of-shipping', __( '    WooCommerce services is not active, ignoring trying to save WooCommerce services labels as cost of shipping.', 'woocommerce-cost-of-shipping' ) );
		}
		return false;
	}
	...
	public function display_wcs_cos_meta_box( $post_id ) {
		...
		$cos = (float)$order->get_meta( '_wc_cost_of_shipping' );
	...
	public function set_label_cost_wcs_callback() {
		...
		do_action( 'wc_cos_before_ajax_set_label_cost_wcs', $_POST['post_id'] );
		
		// Returns true on success, false on anything else
		if( $cost = $this->maybe_save_order_shipping_cost_woocommerce_services( sanitize_text_field( $_POST['post_id'] ) ) ){
			
			do_action( 'wc_cos_after_ajax_set_label_cost_wcs', $_POST['post_id'] );
	...
	private function get_parsed_xml( $xml ) {
		...
		if ( ! $success ) {
			$this->log->add( 'wc-cost-of-shipping', __( 'wpcom_safe_simplexml_load_string(): Error loading XML string in shipstation xml retrieve', 'woocommerce-cost-of-shipping' ) );
			return false;
		}

		if ( isset( $dom->doctype ) ) {
			$this->log->add( 'wc-cost-of-shipping', __( 'wpcom_safe_simplexml_import_dom(): Unsafe DOCTYPE Detected in shipstation xml retrieve', 'woocommerce-cost-of-shipping' ) );
			return false;
		}
	...
	public function show_order_shipping_cost( $post_id ) {
		...
		$cost_of_shipping = (float)get_post_meta( $post_id, '_wc_cost_of_shipping', true );
	...
	protected function get_formatted_order_shipping_cost( $order_id ) {
		...
		$order_shipping_cost = get_post_meta( $order_id, '_wc_cost_of_shipping', true );
